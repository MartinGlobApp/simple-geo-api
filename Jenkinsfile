pipeline {
    ...
    environment {
        ...
        dockerImage = ''
        repositoryName= 'woovix-api-staging'
        gitCommit_2 = ''
    }
    ...
    stages {
        ...
        stage('Build image staging') {
            ...
            environment {
             ...
             steps {
                ...
                echo '*************************'
                script {
                    dockerImage = docker.build("${env.registryAWS}/${env.repositoryName}:${env.GIT_COMMIT}")
                 }
             }
        }
        ...
        stage('Create repository AWS') {
            steps {
                sh "aws ecr describe-repositories --repository-names ${env.repositoryName} || aws ecr create-repository --repository-name ${env.repositoryName}"
            }
        }
        ...
        stage('Remove Image Staging') {
            ...
            steps {
                ...
                echo '*************************'
                sh "docker rmi $registryAWS/$repositoryName:$GIT_COMMIT"
                sh "docker rmi $registryAWS/$repositoryName:latest"
            }
        }
            ...
            steps {
                sshPublisher(publishers:
                    ...
                    execCommand: """
                                  ...
                                  cd /home/silentium/deployer-staging/woovix
    ...
    post {
        ...
        success {
            slackSend botUser: true,
                ...
                iconEmoji: ':beers:',
                color: 'good',
                message: "Job:  ${env.JOB_NAME}\n Status: *SUCCESS*\n URL Staging: woovix-api.staging.silentiumapps.com",
                tokenCredentialId: 'jenkins-bot'
        }
        unstable {
            slackSend botUser: true,
